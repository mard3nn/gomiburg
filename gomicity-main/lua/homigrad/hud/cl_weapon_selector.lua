...
local AcsentColor = Color( 11,11,39, 128)
...
function WS.WeaponSelectorDraw( ply )
    ...
    for i = 0, #Weapons do
        ...
        for Id = 0, #slotTbl do
            ...
            surface.SetDrawColor( 11,11,39, WS.Transparent*( SelectedWep == wep and 200 or 0 )  )
            surface.SetMaterial( gradient_u )
            surface.DrawTexturedRect( position, (scrH * 0.025) * (Ammout) + (scrH * 0.05) + lastPos, sizeX, sizeH )
            if SelectedWep == wep then
                surface.SetDrawColor( 11,11,39, WS.Transparent*155 )
